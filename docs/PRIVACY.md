...
The selected new host is Cloudflare Pages Free, at the live `originscholar.pages.dev` subdomain. No registrar purchase, payment details, paid plan, analytics or application-text service is added. The host serves static files and may receive ordinary request metadata. The packaged `_headers` file requests CSP, referrer restrictions and revalidation; the public response headers were verified after publication (see CHECKS). Historical Sites observations above apply to that old origin, not automatically to Pages.
...
Pages verification: the sampled HTML/JS/CSS/worker bytes matched the build and the requested CSP was present. Cloudflare also returned Network Error Logging headers; operational metadata can reach the provider. This observation is not a promise about all future responses or browser software.
