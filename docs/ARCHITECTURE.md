...
The app currently expects an origin-root deployment over HTTPS. The host must serve JavaScript with the correct content type and honor suitable cache/security headers; `_headers` is a host convention, not a universal standard. The service worker caches only known public app files, never the workspace. Updates require an explicit action and recovery checks before reloading.
