...
Use fictional information in this alpha. Start with [the short demo](DEMO.md), or follow the detailed controls below. Setup commands are in the [README](../README.md).
...
Choose **Full checklist** or **One step at a time**. The one-step view has a task chooser and Previous/Next buttons; completion never advances automatically. Dependencies and review reasons use words and linear lists. **Text size → Larger** increases text size, and browser zoom remains available. See [accessibility notes](ACCESSIBILITY.md) for behavior and limits; [CHECKS](CHECKS.md) identifies what was actually tested.
...
See [dependency design](DEPENDENCIES.md) for the algorithm, migration rules, limits, and an interview explanation.
...
Every nonblank passage stays visible. Positional changed pairs are reading aids, not proven equivalents. Exact snapshots preserve whitespace and offsets. Unlinked spans remain unreviewed even inside partly linked paragraphs. Historical source decisions cannot clear newer reviews or overwrite current applicability. See [comparison design](COMPARISON.md).
