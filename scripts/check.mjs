...
for (const file of [...(await readdir(new URL('../dist/src/', import.meta.url))).filter(file => file.endsWith('.js')).map(file => `dist/src/${file}`), ...evaluationScripts, 'dist/sw.js', 'scripts/serve.mjs', 'scripts/build-release.mjs', 'scripts/check.mjs', 'scripts/evaluate.mjs', 'scripts/evaluation-server.mjs', 'tests/browser-server.mjs', 'tests/accessibility-server.mjs', 'tests/accessibility-diagnostics.js', 'tests/service-worker-diagnostics.js']) {
